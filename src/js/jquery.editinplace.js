...
$.extend(InlineEditor.prototype, {
	...
	connectOpeningEvents: function() {
        ...
        if(that.settings.is_error) 
        {
            that.triggerCallback(that.settings.end_edit); 
            that.settings.is_error = false;  
        }
	},
	...
	saveOriginalValue: function() {
		if (this.settings.use_html)
			...
			if(this.settings.is_error == false) this.originalValue = trim(this.dom.text());
            this.settings.current_value = trim(this.dom.text());
	},
	...
	setInitialValue: function() {
		if(this.settings.is_error) var initialValue = this.triggerDelegateCall('willOpenEditInPlace', this.settings.current_value);
        else var initialValue = this.triggerDelegateCall('willOpenEditInPlace', this.originalValue);
		...
		$.ajax({
			...
			success: function(html){
				var new_text = html || that.settings.default_text;
                 ...
                 if(html.errors.length > 0) that.settings.is_error = true;
                 else that.settings.is_error =false;
